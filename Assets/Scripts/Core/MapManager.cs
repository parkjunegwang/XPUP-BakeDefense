...
public void ApplyWallsFromMapData() { if (mapData == null || mapData.wallPlacements == null || mapData.wallPlacements.Count == 0) return; var tm = TurretManager.Instance; var inv = InventoryManager.Instance; if (tm == null || inv == null) { Debug.LogWarning("[MapManager] TurretManager or InventoryManager not found!"); return; } foreach (var w in mapData.wallPlacements) { var tile = GetTile(w.gridX, w.gridY); if (tile == null) continue; TurretType tt = WallSizeToTurretType(w.wallSize); inv.Add(tt, 1); tm.PlaceSelectedTurret(tile, tt); if (tile.placedTurret != null) tile.placedTurret.isFromMapData = true; } Debug.Log($"[MapManager] Applied {mapData.wallPlacements.Count} wall placements from MapData."); } private TurretType WallSizeToTurretType(MapData.WallSizeType wallSize) { switch (wallSize) { case MapData.WallSizeType.Wall2x1: return TurretType.Wall2x1; case MapData.WallSizeType.Wall1x2: return TurretType.Wall1x2; case MapData.WallSizeType.Wall2x2: return TurretType.Wall2x2; default: return TurretType.Wall; } }

        private void ValidateAndClampPositions() { var validSpawns = new System.Collections.Generic.List<Vector2Int>(); foreach (var p in spawnPositions) { if (p.x >= 0 && p.x < columns && p.y >= 0 && p.y < rows) validSpawns.Add(p); else Debug.LogWarning($"[MapManager] Spawn ({p.x},{p.y}) is out of range! Map is {columns}x{rows}. Ignoring."); } var validEnds = new System.Collections.Generic.List<Vector2Int>(); foreach (var p in endPositions) { if (p.x >= 0 && p.x < columns && p.y >= 0 && p.y < rows) validEnds.Add(p); else Debug.LogWarning($"[MapManager] End ({p.x},{p.y}) is out of range! Map is {columns}x{rows}. Ignoring."); } if (validSpawns.Count == 0) { validSpawns.Add(new Vector2Int(columns / 2, rows - 1)); Debug.LogWarning($"[MapManager] No valid spawns! Using default: ({columns/2},{rows-1})"); } if (validEnds.Count == 0) { validEnds.Add(new Vector2Int(columns / 2, 0)); Debug.LogWarning($"[MapManager] No valid ends! Using default: ({columns/2},0)"); } spawnPositions = validSpawns; endPositions = validEnds; }
}
}
