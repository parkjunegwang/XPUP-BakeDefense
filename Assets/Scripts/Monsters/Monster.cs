...
namespace Underdark
{
    public class Monster : MonoBehaviour
    {
        ...
        /// <summary>지난 프레임 위치 - 스윗 판정용</summary>
...
private void Update() { if (!IsAlive) return; if (GameManager.Instance == null) return; if (GameManager.Instance.CurrentState != GameState.WaveInProgress) return; LastPosition = transform.position; if (_slowTimer > 0f) { _slowTimer -= Time.deltaTime; if (_slowTimer <= 0f) { speed = _baseSpeed; if (bodyRenderer != null) UpdateSortingOrder(); } } if (_pathIdx >= _path.Count) return; bool isLast = (_pathIdx == _path.Count - 1); Vector3 currentWP = isLast ? _path[_pathIdx] : _path[_pathIdx] + _navOffset; float distToCurrent = Vector3.Distance(transform.position, currentWP); if (!isLast && distToCurrent < 0.35f) { _pathIdx++; return; } if (isLast && distToCurrent < 0.08f) { ReachEnd(); return; } Vector3 moveTarget = currentWP; if (!isLast && _pathIdx + 1 < _path.Count) { bool nextIsLast = (_pathIdx + 1 == _path.Count - 1); Vector3 nextWP = nextIsLast ? _path[_pathIdx + 1] : _path[_pathIdx + 1] + _navOffset; float blend = Mathf.Clamp01(1f - distToCurrent / 0.6f); moveTarget = Vector3.Lerp(currentWP, nextWP, blend * 0.6f); } Vector3 dir = moveTarget - transform.position; if (Mathf.Abs(dir.x) > 0.01f && bodyRenderer != null) bodyRenderer.flipX = dir.x < 0; transform.position = Vector3.SmoothDamp(transform.position, moveTarget, ref _smoothVel, 0.08f, speed * 3f); UpdateSortingOrder(); }

public void TakeDamage(float dmg, bool isCrit = false) { if (!IsAlive) return; _hp -= dmg; RefreshHpBar(); if (_animator != null) _animator.SetTrigger("Hit"); float now = Time.time; if (now - _lastPopupTime > 0.12f) { _lastPopupTime = now; DamagePopup.Create(transform.position, dmg, isCrit); } if (_hp <= 0f) Die(); }

private void Die() { _hp = 0f; if (_animator != null) { _animator.SetTrigger("Dead"); } GameManager.Instance.OnMonsterKilled(reward); MonsterManager.Instance.OnMonsterDied(this); StartCoroutine(DieAnim()); }
